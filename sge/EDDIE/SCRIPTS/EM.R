...
run.em <- function( Adj, obs, meas, tol=1e-5, max.steps=1e3, restarts=10,
    ...
    for( r in 1:restarts ){
        ...
        if( res@emTESTS[1] == 1 ){
            ...
            res <- em( Adj=Adj, Nij=Nij, Eij=Eij, Qij=res@Qij, params=res@params, modes=modes, tol=tol,
                      max.steps=max.steps, initPARAMS=res@params, fixPARAMS=fix_all_params(res@params),
                      constPARAMS=constPARAMS, store.delta.N=store.delta.N, e.first=e.first,
                      conv.params=conv.params, SMALL=SMALL )
            cat(" done.\n")
