        ...
        for( j in 1:n ){
            ...
            if( i < j ){
                ...
                ll = ll + aij * adj.log(     rho,  epsilon=epsilon ) +
                    ...
                    aij * eij * adj.log(   alpha,  epsilon=epsilon ) +
            aij * (nij - eij) * adj.log((1-alpha), epsilon=epsilon ) +
                (1-aij) * eij * adj.log(    beta,  epsilon=epsilon ) +
        (1-aij) * (nij - eij) * adj.log( (1-beta), epsilon=epsilon )
...
em <- function( Adj, Nij, Eij, Qij, params, tol, max.steps, initPARAMS, fixPARAMS, epsilon ){
...
run.em <- function( Adj, obs, Nmeas=c(4,8,16), study=1, errRATE, tol=1e-5, max.steps=1e3, restarts=10, initPARAMS=NULL, fixPARAMS=NULL, epsilon=NULL ){
