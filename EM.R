...
Qij.param <- function( meas, obs, theta ){
    ...
    if( num == 0 && dem == 0 ){ Qij = 0 }

    if( num > 0  && dem == 0 ){ Qij = 0 } 
    
    if( num > 0  && dem > 0  ){ Qij = num / dem }
...
em <- function( Adj, Nij, Eij, Qij, params, tol, max.steps, initPARAMS ){
    ...
    if( is.null(initPARAMS) ){
        ...
        params <- init_params(initPARAMS, c(0,0,0)) 
    ...
    while( foundMAX == 0 ) {
        ...
        old.ll     <- posterior( gg=Adj, QQ=Qij )
        ...
        new.ll     <- posterior( gg=Adj, QQ=Qij )
        ...
        if( abs(old.ll) < abs(new.ll) ){ foundMAX = 1 }
        cat(" LL_old ", old.ll ," LL_new ", new.ll, " params ", unlist(params)  ,"\n")
        ...
        if( steps > max.steps ){
         ...
         #--- test if our current ll for nan?
        if( is.nan(new.ll) ){
            emTEST2 = 1
...
run.em <- function( Adj, meas, obs, Nmeas=c(4,8,16), study=1, errRATE, tol=1e-3, max.steps=1e3, restarts=10, initPARAMS=NULL ){
    ...
    res <- em( Adj=Adj, Nij=Nij, Eij=Eij, Qij=Q, params=params, 
               tol=tol, max.steps=max.steps, initPARAMS=initPARAMS )
